...
import { DeleteNoteButton } from "../components/submit_button";
    ...
    <div className="grid items-start gap-y-8">
      <div className="flex items-center justify-between px-2">
        ...
        {data?.Subscription?.status === "active" ? (
          <Button asChild>
            <Link href="/dashboard/new_note">Create a new Note</Link>
          </Button>
        ) : (
          <Button asChild>
      ...
      {data?.Notes.length == 0 ? (
        <div className="flex min-h-[400px] flex-col items-center rounded-md border border-dashed p-8 text-center animate-in fade-in-50">
          ...
          {data?.Notes.map((item: any) => (
            ...
            >
              ...
              <div className="flex gap-x-4">
                <Link href={`/dashboard/new_note/${item.id}`}>
                  <Button variant="outline" size="icon">
                    <Edit className="w-4 h-4" />
                  </Button>
                </Link>
                <form action={DeleteNote}>
                  <Input className="hidden" name="noteId" value={item.id} />
                  <DeleteNoteButton />
                </form>
              </div>
            </Card>
